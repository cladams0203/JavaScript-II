...
// Problem 1
// A thank you for your donation message to the company representitive.
runners.forEach((element) => {
    console.log( `We would like to thank ${element.first_name} ${element.last_name} of ${element.company_name} for their \$${element.donation} donation!`);
});

// Problem 2
// Average donation ammount.
let averageDonation = [];
averageDonation = runners.reduce((total, element) => {
    return total += element.donation;
}, 0);
averageDonation = averageDonation / runners.length;
console.log(averageDonation);

// Problem 3
// Shirts to order for the event.
let xs = runners.filter((element) => {
    return element.shirt_size === `XS`;
  });
  let s = runners.filter((element) => {
    return element.shirt_size === `S`;
  });
  let m = runners.filter((element) => {
    return element.shirt_size === `M`;
  });
  let l = runners.filter((element) => {
    return element.shirt_size === `L`;
  });
  let xl = runners.filter((element) => {
    return element.shirt_size === `XL`;
  });
  let xxl = runners.filter((element) => {
    return element.shirt_size === `2XL`;
  });
  let xxxl = runners.filter((element) => {
    return element.shirt_size === `3XL`;
  });
  console.log(`XS:${xs.length}, S:${s.length}, M:${m.length}, L:${l.length}, XL:${xl.length}, 2XL:${xxl.length}, 3XL:${xxxl.length}`);